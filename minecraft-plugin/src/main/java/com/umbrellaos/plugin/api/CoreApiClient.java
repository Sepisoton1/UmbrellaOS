...
public class CoreApiClient {
    ...
    private static final Map<String, String> ANALYTICS_TYPE_MAP = new java.util.HashMap<>();
    static {
        ANALYTICS_TYPE_MAP.put("player_join", "join");
        ANALYTICS_TYPE_MAP.put("player_leave", "quit");
        ANALYTICS_TYPE_MAP.put("player_death", "death");
        ANALYTICS_TYPE_MAP.put("player_kill", "kill");
    }

    public CompletableFuture<Void> postEvent(String type, String playerUuid, String message, Map<String, Object> metadata) {
        String analyticsType = ANALYTICS_TYPE_MAP.get(type);
        if (analyticsType != null && playerUuid != null) {
            JsonObject body = new JsonObject();
            body.addProperty("event_type", analyticsType);
            body.addProperty("minecraft_uuid", playerUuid);
            if (metadata != null) {
                body.add("data", gson.toJsonTree(metadata));
            }
            return asyncPost("/api/v1/analytics/events", body).thenApply(response -> null);
        }
        JsonObject body = new JsonObject();
    ...
    public CompletableFuture<String> requestVerification(String playerUuid, String username, String ipAddress) {
        ...
        body.addProperty("player_username", username);
